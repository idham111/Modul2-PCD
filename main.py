...
async def upload_image(request: Request, file: UploadFile = File(...)):
    ...
    if img is None:
        return HTMLResponse("File yang diunggah bukan gambar yang valid.", status_code=400)
...
async def perform_logic_operation(
    ...
    if operation == "not":
        ...
        if img1.shape != img2.shape:
            return HTMLResponse("Ukuran kedua gambar harus sama untuk operasi AND/XOR.", status_code=400)
        if operation == "and":
